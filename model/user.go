...
type User struct {
	...
	InviterRewardUnlockedByPayment bool           `json:"inviter_reward_unlocked_by_payment" gorm:"default:false;column:inviter_reward_unlocked_by_payment"`
...
func inviteUser(inviterId int) (err error) {
	return recordUserInvitation(inviterId, common.QuotaForInviter)
}
...
func recordUserInvitationTx(tx *gorm.DB, inviterId int, quotaForInviter int) error {
	...
	if quotaForInviter > 0 {
		updates["aff_quota"] = gorm.Expr("aff_quota + ?", quotaForInviter)
		updates["aff_history"] = gorm.Expr("aff_history + ?", quotaForInviter)
	}
...
func (user *User) Edit(updatePassword bool) error {
	...
	newUser := *user
	if err := ValidateReferralCommissionPercent(newUser.ReferralCommissionPercent); err != nil {
		return err
	}
	updates := map[string]interface{}{
...
func ValidateReferralCommissionPercent(percent *float64) error {
	if percent == nil {
		return nil
	}
...
func IncreaseUserQuota(id int, quota int, db bool) (err error) {
	...
	if common.RedisEnabled {
		gopool.Go(func() {
			err := cacheIncrUserQuotaEnabled(id, int64(quota))
			if err != nil {
...
func DecreaseUserQuota(id int, quota int, db bool) (err error) {
	...
	if common.RedisEnabled {
		gopool.Go(func() {
			err := cacheDecrUserQuotaEnabled(id, int64(quota))
			if err != nil {
