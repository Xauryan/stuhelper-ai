...
func StuHelperAIErrorFromParamOverride(err *ParamOverrideReturnError) *types.StuHelperAIError {
	if err == nil {
	...
	opts := make([]types.StuHelperAIErrorOptions, 0, 1)
	if err.SkipRetry {
...
//
// 旧实现把整个 jsonData unmarshal 成 map[string]interface{} 再 marshal
// 回来。这里改为直接在 []byte 上按顶层字面 key 写入，避免大请求体被完整
// 解码和重新编码。
