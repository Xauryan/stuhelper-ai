...
// Configuration (read once from env; conservative defaults). The breaker only
// excludes channels from selection and auto-recovers, so it is safe to default
// on; set CHANNEL_BREAKER_ENABLED=false to disable entirely.
var (
	breakerEnabled        = common.GetEnvOrDefaultBool("CHANNEL_BREAKER_ENABLED", true)
	breakerWindow         = time.Duration(common.GetEnvOrDefault("CHANNEL_BREAKER_WINDOW_SECONDS", 120)) * time.Second
	breakerMinSamples     = common.GetEnvOrDefault("CHANNEL_BREAKER_MIN_SAMPLES", 10)
	breakerTripScorePct   = common.GetEnvOrDefault("CHANNEL_BREAKER_TRIP_SCORE_PCT", 50)
	breakerConsecFatal    = common.GetEnvOrDefault("CHANNEL_BREAKER_CONSECUTIVE_FATAL", 3)
	breakerCooldown       = time.Duration(common.GetEnvOrDefault("CHANNEL_BREAKER_COOLDOWN_SECONDS", 60)) * time.Second
	breakerMaxCooldown    = time.Duration(common.GetEnvOrDefault("CHANNEL_BREAKER_MAX_COOLDOWN_SECONDS", 1800)) * time.Second
	breakerHalfOpenProbes = common.GetEnvOrDefault("CHANNEL_BREAKER_HALFOPEN_PROBES", 3)
)
