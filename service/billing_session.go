...
// 任一步骤失败时原子回滚已完成的步骤。
func (s *BillingSession) preConsume(c *gin.Context, quota int) *types.StuHelperAIError {
	...
	if err := s.funding.PreConsume(effectiveQuota); err != nil {
		...
		if strings.Contains(errMsg, "no active subscription") ||
			...
			strings.Contains(errMsg, "no subscription allows model") {
...
func NewBillingSession(c *gin.Context, relayInfo *relaycommon.RelayInfo, preConsumedQuota int) (*BillingSession, *types.StuHelperAIError) {
	if relayInfo == nil {
	...
	// 钱包路径需要先检查用户额度
	tryWallet := func() (*BillingSession, *types.StuHelperAIError) {
	...
	trySubscription := func() (*BillingSession, *types.StuHelperAIError) {
