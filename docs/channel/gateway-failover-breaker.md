...
管理员也可以在 classic 渠道列表中对 `open` / `half_open` 渠道执行“重置熔断”，或直接调用 `POST /api/channel/:id/breaker/reset`。该操作只清理处理该请求的服务进程内存中的 breaker 状态和最近 `availability` telemetry，不修改 `channels.status`、不启用或禁用渠道、不刷新渠道缓存，也不向其它实例广播。重置后该实例会把 `breaker_state` 返回为 `closed`，该渠道能否被选中仍取决于数据库渠道状态、分组、模型能力、权重、优先级和其它选路条件。

多实例部署时，手动重置只对接收该请求的实例生效。如果每个实例都可能独立熔断同一渠道，需要逐实例执行重置，或未来将 breaker 状态外置到共享存储；在当前实现里不能把一次 API 调用视为全局恢复。
...
当 `breaker_state` 为 `open` 或 `half_open` 时，classic 渠道列表的更多菜单会显示“重置熔断”。确认后前端调用 `POST /api/channel/:id/breaker/reset`，并仅用返回值更新当前行的 `breaker_state` 和 `availability`。该按钮不等价于“启用渠道”，不会改变数据库中的渠道启用状态。

## 渠道可用性监控

经典后台新增独立的可用性监控聚合接口，用于给 classic 的模型广场和仪表盘展示持久化可用率。管理员兼容入口是 `GET /api/channel/monitor/summary`；审计只读入口是 `GET /api/log/channel_monitor/summary`，供审计管理员在主页查看健康度。该接口读取既有 `logs` 表，不新增监控表，也不参与渠道选择、熔断、自动禁用或计费。

审计管理员只能看到渠道数字 ID。后端不会为审计管理员回填 `channel_name`，classic 表格也只渲染 `#渠道ID`，避免通过主页可用性检测泄露渠道名称、标签、供应商或其它具体渠道信息。
