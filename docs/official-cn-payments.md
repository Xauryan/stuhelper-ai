...
- 超时订单的本地状态推进改为完全异步，不再阻塞账单页请求：
  ...
  - 非 master 节点的 ticker 内部直接返回，但管理员账单列表
    ...
    兜底，借助 `TryLock` 互斥保证同节点同一时刻只有一份清理在跑。这样无论
    部署是否包含 master 节点，本地 pending 订单都能在合理时间内被推进到
    `expired`。
...
当前筛选范围内的支付总金额（`total_money`），不是当前页金额临时相加。
...
和搜索结果都以数据库过滤结果为准，不做前端假分页。账单列表的 COUNT 与
SUM 与 SELECT 包在一次 `DB.Transaction(ReadOnly=true)` 里执行，PostgreSQL/MySQL
走 RepeatableRead 隔离级别，SQLite 走默认（WAL 模式下事务即快照），保证同一次
HTTP 请求看到的 `total`、`total_money` 与 `items` 一致；跨页翻页仍是最终一致语义。
