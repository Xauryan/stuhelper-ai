...
订阅订单的最多可退金额按实际额度已用比例计算，不再因为进入某个重置周期就直接
扣减未使用订阅金额。系统仍会在预览中返回时间比例和额度比例，方便管理员判断，
但退款上限只按 `已用额度 / 总额度` 约束：预设退款金额为有效支付金额乘以
`1 - 已用额度比例`，扣除该订单已退款金额后向下保留两位小数。订阅退款不会扣
用户钱包余额；只要退款成功，即使只是部分退款，也会取消该订单对应的订阅实例并
按订阅分组规则回退用户分组，账单和订阅订单状态则继续按实际退款金额显示为
`partial_refunded` 或 `refunded`。例如 58 元 7 日订阅、每日重置，用户完全未使用
额度时最多可退 58.00 元；若已使用 20% 额度，则最多可退 `80% * 58 = 46.40` 元；
若实际支付 58.35 元，其中 0.35 元手续费不参与退款。
...
审批通过或直接退款后，系统按订阅实际未用额度比例计算可退金额，并按购买时
