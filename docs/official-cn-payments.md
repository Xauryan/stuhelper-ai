...
支付并在 classic 弹窗中展示 `code_url` 二维码，移动端使用微信 H5 支付并跳转
`h5_url`。微信支付异步通知仍使用 `/api/wechat-pay/official/notify`，回调处理
会先识别微信支付官方订阅订单并完成订阅；如果不是订阅订单，再按普通额度充值
处理。
...
订阅订单完成后会写入同一张充值账单；classic 会把 `SUB`、`ALIPAYSUB`、
`WXSUB` 前缀且充值额度为 0 的账单识别为“订阅套餐”，避免官方订阅账单显示成
普通 0 额度充值。
