...
支付通道的手续费百分比计算实际向支付网关收取的总金额，同样向上保留到分。

`top_ups.money` 和 `subscription_orders.money` 表示用户账单、充值排行、
返佣和退款所使用的有效支付金额；新增的 `fee` 字段表示不可退手续费。对外支付
网关创建订单、官方支付回调金额校验和微信退款 `amount.total` 使用
`money + fee`，但退款预览、退款上限、退款比例、返佣冲回和账单“支付金额”都只
按 `money` 计算。旧订单的 `fee` 默认为 `0`，仍按原有金额语义处理。

管理员在用户管理中选择“充值”调整额度时，也会创建 `admin_add`/`admin`
充值账单。该账单的 `money` 按“原生额度 / `QuotaPerUnit` *
`AlipayOfficialUnitPrice`”计算并向上保留两位小数，`fee` 按
`AlipayOfficialServiceFeePercent` 计算，语义与官方支付宝有效支付金额和不可退
手续费保持一致。历史零金额管理员充值账单会在支付配置加载后回填 `money` 和
`fee`，避免迁移早于配置加载时使用默认价格。
...
充值订单尚未退回的额度，再与用户当前余额取较小值，按有效支付金额比例
折算为可退人民币并向下保留两位小数。例如用户原有余额 100，又充值 100 后只
使用 50，当前余额仍覆盖这笔订单的 100 额度，则预设可全额退款；如果之后总共
使用 150，当前余额只剩 50，则这笔充值预设最多退款 50 对应的人民币。
手续费不进入可退金额，例如订单实际支付 20.12 元，其中 `money=20.00`、
`fee=0.12`，最多只能按 20.00 元计算退款。

订阅订单的最多可退金额按重置周期结算权益。未开始的订阅可全额退款，已结束的
订阅不可退款；无重置周期的订阅继续按 `1 - 已用额度比例` 计算未使用部分。存在
每日、每周、每月或自定义重置周期时，系统按订阅开始到结束之间的总重置周期数
拆分有效支付金额：已经结束的周期不退，当前周期按 `1 - 当前周期已用额度比例`
退款，未来完整周期全额退款。退款上限为
`有效支付金额 * ((未来完整周期数 + 当前周期未用额度比例) / 总周期数)`，再扣除
该订单已退款金额并向下保留两位小数。系统仍会在预览中返回时间比例和额度比例，
方便管理员判断。订阅退款不会扣用户钱包余额；只要退款成功，即使只是部分退款，
也会取消该订单对应的订阅实例并按订阅分组规则回退用户分组，账单和订阅订单状态
则继续按实际退款金额显示为 `partial_refunded` 或 `refunded`。例如 70 元 7 日
订阅、每日重置，第 3 天退款且当前周期已使用 50% 额度时，未来完整周期 4 个、
当前周期可退 50%，最多可退 `(4 + 0.5) / 7 * 70 = 45.00` 元；若实际支付金额中
包含手续费，手续费不参与退款。
...
管理员充值账单的退款只做本地记账和额度扣回，不调用第三方支付接口。退款金额
按管理员充值账单的有效支付金额 `money` 与退款额度比例折算；手续费 `fee` 不
进入退款金额。最后一次退完剩余额度时，会把剩余有效金额全部记入退款，避免按
比例向下取整留下 0.01 元尾差。
...
余额购买订阅也支持退款申请和管理员直接退款，但不调用支付宝、微信或其他第三方
接口。用户在账单管理中对 `balance` 支付方式的订阅订单发起退款申请，管理员
审批通过或直接退款后，系统按同一套重置周期退款规则计算可退金额，并按购买时
记录的 `charged_quota` 比例返还用户钱包余额。任意成功退款都会取消该订单对应
的订阅实例并按订阅分组规则回退用户分组；订阅订单和账单状态继续按实际退款金额
标记为 `partial_refunded` 或 `refunded`。普通 `balance` 非订阅账单不开放退款。
...
- 微信 Native 二维码弹窗只展示二维码和支付状态，不再展示订单号。二维码使用
  ...
  “支付完成后将自动刷新”使用静态等待状态，加载图标在订单未超时时持续连续
  旋转，不再随单次轮询请求开始或结束停顿，也不再使用闪烁提示。
...
classic 后台“订阅管理”还提供“允许余额支付”开关，对应套餐字段
`allow_balance_pay`。该开关只控制用户是否可以用钱包余额兑换订阅，不影响
...
误拦截用户。
同一用户重复购买同一订阅套餐时，新的订阅实例会从该套餐当前最后一个有效期末尾
开始，作为续期排队，不会和现有同套餐订阅时间重叠。未来开始的续期记录会在订阅
列表中保留并显示为未开始，但接口计费只会消耗 `start_time <= now < end_time` 的
当前生效订阅；带升级分组的续期也只会在续期实际开始后恢复用户分组。
订阅支付订单会在 `provider_payload` 中记录对应 `subscription_id`，退款时优先按
该实例计算和取消，避免多次购买同一套餐后退款误命中其他有效期。
...
classic 会把 `SUB`、`SUBBAL`、`BALANCE__`、`ALIPAYSUB`、`WXSUB`、`SSSUB`
前缀且充值额度为 0 的账单识别为“订阅套餐”，避免订阅账单显示成普通 0 额度
充值。新生成的余额订阅订单使用 `BALANCE__{user_id}_{random}`；自助微信订阅
使用 `WXSUB_SS_{user_id}_{random}`，自助支付宝订阅使用
`ALIPAYSUB_SS_{user_id}_{unix_milli}_{random}`。微信支付相关订单仍限制在微信
商户订单号 32 字符以内。
...
当前筛选范围内状态为 `success` 的有效支付成功金额（`total_money`，仅汇总
`money`），不是当前页金额临时相加；待支付、失败、超时、部分退款和已退款账单
仍可出现在列表中，但不计入该金额。账单“支付金额”列显示有效支付金额，旁边的
“手续费”列显示不可退手续费，例如有效支付 10.00 元、手续费 0.06 元会拆成两列
展示。
...
审核中”。余额购买订阅、已审核通过的自助支付成功订单也使用同一套退款申请入口；自助支付
申请退款时用户必须提交退款收款码，余额购买订阅不需要收款码。管理员账单对待
审核申请显示“审批退款”和“拒绝”；直接退款仍保留在没有待审核申请的官方支付
订单、余额订阅订单和已审核通过的自助支付订单上。自助支付直接退款只登记本地
人工退款，不调用支付宝或微信退款接口。退款确认弹窗会展示预设可退金额、订阅
未用额度比例、用户申请金额和申请原因。
管理员充值账单额外显示“编辑”操作。管理员可以修改充值额度、有效支付金额、
手续费和手续费比例；默认会按支付宝官方充值价格与手续费配置重新计算。编辑时
也可以把管理员充值转换为管理员赠送：系统会删除对应 `top_ups` 账单、把对应
日志改为“管理员赠送用户额度”，因此该记录会从账单管理和充值排行中移除，支付
金额与手续费归零。已有退款记录的管理员充值账单不可再编辑，避免账单金额、退款
金额和用户余额发生二次不一致。
