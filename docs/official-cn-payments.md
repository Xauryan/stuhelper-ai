...
SELECT 包在一次 `DB.Transaction(ReadOnly=true)` 里执行，PostgreSQL/MySQL 走
RepeatableRead 隔离级别，SQLite 走默认（WAL 模式下事务即快照），保证同一次
HTTP 请求看到的 `total` 与 `items` 一致；跨页翻页仍是最终一致语义。
