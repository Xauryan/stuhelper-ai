...
官方支付宝同时接入“额度充值”和“订阅套餐购买”。订阅套餐购买弹窗会把
`alipay_official` 作为独立官方支付宝方式展示，不再依赖易支付的
`enable_online_topup` 开关；如果只配置了支付宝官方支付、没有配置易支付，
订阅套餐仍会显示“支付宝”支付入口。订阅订单通过
`POST /api/subscription/alipay-official/pay` 创建，电脑端使用
`alipay.trade.page.pay`，移动端使用 `alipay.trade.wap.pay`。订阅价格按套餐
美元金额乘以 `AlipayOfficialUnitPrice` 换算为人民币，并按进一法保留两位
小数提交给支付宝。支付宝异步通知仍使用 `/api/alipay/official/notify`，回调
处理会先识别官方支付宝订阅订单并完成订阅；如果不是订阅订单，再按普通额度充值
处理。
微信支付官方目前只接入“额度充值”。订阅套餐购买弹窗不会把 `wxpay_official`
当成易支付子渠道使用。
