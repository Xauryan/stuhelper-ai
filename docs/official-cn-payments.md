...
订阅订单的最多可退金额按时间和额度两条线取更严格的已使用比例。时间比例会先
匹配套餐的额度重置周期：每日、每周、自定义周期都按“已过周期数向上取整 /
总周期数”计算，不足一个重置周期也按完整周期使用；每月重置按日历月周期计数；
不重置的订阅退回到连续时间比例。最终 `已使用比例 =
max(重置周期时间比例, 已用额度 / 总额度)`，预设退款金额为有效支付金额乘以
`1 - 已使用比例`，并向下保留两位小数。订阅部分退款不会扣用户钱包余额；全额
退款会取消对应订阅实例并按订阅分组规则回退用户分组。例如 58 元 7 日订阅、
每日重置，用户 2026-05-20 13:00 购买，2026-05-23 17:00 申请退款，已过
3 天 4 小时会按 4 个每日周期计，最多可退 `3/7 * 58 = 24.85` 元；若实际支付
58.35 元，其中 0.35 元手续费不参与退款。
...
审批通过或直接退款后，系统按订阅未使用时间/额度比例计算可退金额，并按购买时
记录的 `charged_quota` 比例返还用户钱包余额。部分退款会把订阅订单和账单标记为
`partial_refunded` 并保留订阅；全额退款会标记为 `refunded`、取消订阅并按订阅
分组规则回退用户分组。普通 `balance` 非订阅账单不开放退款。
...
classic 会把 `SUB`、`SUBBAL`、`ALIPAYSUB`、`WXSUB`、`SSSUB` 前缀且充值额度为
0 的账单识别为“订阅套餐”，避免订阅账单显示成普通 0 额度充值。系统生成的
订阅订单号统一使用 `{prefix}_{user_id}_{unix_milli}_{random}` 格式；微信支付
官方订单仍使用同类短格式并限制在微信商户订单号 32 字符以内。
