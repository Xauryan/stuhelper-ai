...
- 相关参考：
  ...
  - 微信支付 API v3 Native 支付 / 支付成功通知文档。
...
- 微信支付官方订阅订单复用微信支付 API v3，电脑端使用
  `/v3/pay/transactions/native` 并展示扫码二维码；移动端在创建订单前直接提示
  当前不支持微信支付，要求使用电脑端或其他支付方式，避免 H5 未开通时落到
  “拉起支付失败”。
- 官方支付宝和微信支付官方各自使用秒级订单有效期配置。支付宝使用
  `AlipayOfficialOrderTimeoutSec`，微信使用 `WechatPayOfficialOrderTimeoutSec`；
  旧 `AlipayOfficialOrderTimeoutMin` 只作为历史数据兼容。微信扫码弹窗隐藏订单
  号，使用单一二维码边框和倒计时等待状态。
- 账单列表读取前会按官方支付宝和微信支付官方各自的秒级订单有效期，把超时后仍
  处于待支付的官方订单本地标记为已超时；已超时的官方订单仍保留管理员查询入口，
  官方充值和官方订阅订单仍保留补单入口，查询成功、支付通知或补单仍可把订单
  推进为成功。
...
- 用户可在自己的 classic 充值账单中对官方支付宝/微信支付成功或部分退款订单
  ...
  额度充值按当前用户余额和该订单剩余额度取较小值折算人民币，订阅按重置周期
  结算权益，已结束周期不退，当前周期按未用额度比例退，未来完整周期全额退。
  管理员可审批通过后调用官方退款 API，也可拒绝申请；管理员直接退款仍可选择
  全额退款。
- 订阅退款不会扣用户钱包余额。只要退款成功，即使只是部分退款，也会取消该订单
  对应的订阅实例、按分组规则回退用户分组；订阅订单和账单仍按实际退款金额显示为
  `partial_refunded` 或 `refunded`，并按 `subscription` 来源和订阅订单 ID 冲回
  返佣。额度充值退款仍按 `topup` 来源冲回返佣。
