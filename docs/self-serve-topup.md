...
管理员审批自助退款，或对已审核通过且没有待处理退款申请的自助支付订单直接点击“退款”时，都不会调用支付宝或微信退款接口，只会登记一条本地成功退款记录。额度充值退款会扣回用户余额并冲回返佣；订阅退款会同步订阅订单状态，部分退款保留订阅，满足全额退款条件时取消订阅并回退分组。拒绝退款申请只更新申请状态，不扣余额、不调用外部接口。
