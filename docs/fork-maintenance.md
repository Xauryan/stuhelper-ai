...
- `audit_admin` 是 StuHelper AI 本地只读审计管理员角色，角色值为 `5`。该角色
  可进入 classic 管理区中的用户管理、兑换码管理、模型管理、渠道管理和订阅管理，
  但只能查看列表和执行查询。它不得看到系统设置、模型部署、邀请管理、充值订单
  管理、用户详情、渠道详情、兑换码详情、模型详情、供应商详情或任何新增、编辑、
  删除、启用、禁用、测试、复制、同步、批量操作、订阅绑定、重置 Passkey、重置
  2FA、注销等写操作入口。后端必须使用 `AuditAdminAuth` 只开放列表/搜索接口，并
  使用 `RequireAdminRole` 保护所有详情和写接口；列表响应必须剥离密钥、Base URL、
  覆写配置、备注、用户邮箱、OAuth 标识、访问 token、Stripe 客户号和单用户返佣
  覆盖等敏感字段，避免只靠 classic 前端隐藏按钮。管理员可在 classic 用户管理
  创建审计管理员账号，或把普通用户设为审计管理员；超级管理员仍是唯一可把用户
  提升为普通管理员的角色。
