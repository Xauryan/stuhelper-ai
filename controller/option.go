...
func isVisiblePublicKeyOption(key string) bool {
	switch key {
	case "WaffoPancakeWebhookPublicKey", "WaffoPancakeWebhookTestKey", "AlipayOfficialAlipayPublicKey", "WechatPayOfficialPlatformPublicKey":
		return true
	default:
		return false
	}
...
func UpdateOption(c *gin.Context) {
	...
	case "theme.frontend":
		if option.Value != "classic" {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "无效的主题值，当前仅支持 classic（经典前端）",
			})
			return
		}
	case "GroupRatio":
	...
	case "channel_affinity_setting.temporary_error_status_codes":
		_, err = operation_setting.ParseHTTPStatusCodeRanges(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
